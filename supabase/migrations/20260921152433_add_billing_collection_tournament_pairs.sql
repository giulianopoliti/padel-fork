...
BEGIN
  ...
  INSERT INTO public.billing_collection_tournaments (
    collection_id, tournament_id, amount_ars, tournament_name, club_name, billable_units
  )
  SELECT v_collection_id, value.tournament_id, value.amount_ars, value.tournament_name, value.club_name, value.billable_units
  FROM jsonb_to_recordset(p_tournaments) AS value(
    tournament_id uuid,
    amount_ars integer,
    tournament_name text,
